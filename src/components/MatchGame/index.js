...
class MatchGame extends Component {
  ...
  render() {
    const {tabId, randomIndex, score, timer} = this.state
    console.log(timer)
    const filteredItems = imagesList.filter(each => each.category === tabId)
        ...
        <div className="bottom-container">
          <div>
            <img
              ...
              alt={imagesList[randomIndex].category}
            />
          </div>
          ...
          <ul className="unordered-list">
            {filteredItems.map(each => (
              <MatchGameItem
                key={each.id}
                id={each.id}
                thumbnailImage={each.thumbnailUrl}
